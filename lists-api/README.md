...
| `SKU` | No | Product count filter (e.g. `1000|GTE`) |
| `EMPLOYEES` | No | Employee count filter (e.g. `50|GTE`) |
| `FOLLOWERS`, `SITEMAP`, `PAGERANK`, `BWRANK`, `TRANCO`, `MAJESTIC`, `BWS`, `ECAT`, `AIM`, `AIO`, `AIR`, `AIV` | No | Additional numeric attribute filters |
...
Numeric filters accept `number|operator`, where operator is `EQ`, `LT`, `LTE`, `GT`, or `GTE`. If the operator is omitted, `GTE` is used. Attribute filters are combined with AND.
...
- **Method**: GET
- **Parameters**: `KEY`, `TECH`, `OTHERTECHS`, `META`, `COUNTRY`, `OFFSET`, `SINCE`, `ALL`, `SPEND`, `REVENUE`, `SKU`, `FOLLOWERS`, `EMPLOYEES`, `SITEMAP`, `PAGERANK`, `BWRANK`, `TRANCO`, `MAJESTIC`, `BWS`, `ECAT`, `AIM`, `AIO`, `AIR`, `AIV`
